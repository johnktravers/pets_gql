...
const Mutation = new GraphQLObjectType({
  ...
  fields: {
    addOwner: {
      ...
      resolve(parent, args) {
        return database('owners').returning('*').insert({
          name: args.name,
          age: args.age
        })
    ...
    deleteOwner: {
      ...
      resolve(parent, args) {
        return database('owners').where('id', args.id).del()
        .then(result => {
          if (result == 1) {
            return 'Success!';
          } else {
    ...
    addPet: {
      ...
      resolve(parent, args) {
        return database('pets').returning('*').insert({
          name: args.name,
    ...
    deletePet: {
      ...
      args: {id: {type: GraphQLNonNull(GraphQLID)}},
      resolve(parent, args) {
        return database('pets').where('id', args.id).del()
        .then(result => {
          if (result == 1) {
            return 'Success!';
          } else {
